...
class Math(Evaluator):
    ...
    def load_data(self, split: str) -> Dataset:
        ...
        if split == Split.TEST:
            ...
            data = self.load_jsonl(file_path=os.path.join(DATA_DIR, "train.json"))
        else:
    ...
    def local_evaluate(self, model_name_or_path: str, lora_path: str, split: str, **kwargs) -> float:
            ...
            if lora_path == "base":
                ...
                outputs = llm.generate(
                    ...
                    lora_request=LoRARequest(
                        lora_name=f"math_{lora_path.split('/')[-1]}", 
                        lora_int_id=1, 
                        lora_path=lora_path,
                    )
                )
        ...
        with open(f"results/math/{lora_path.split('/')[-1]}_{split}_results.json", "w") as f:
